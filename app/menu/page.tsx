...
export default function PublicMenu() {
  ...
  const pColor = restaurant.primary_color || '#1e3a8a'; // Orijinal resimdeki lacivert tonu
                ...
                {cart.map((item) => (
                  <div key={item.id} className="p-3 rounded-2xl flex gap-3 items-center border bg-white border-slate-100 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-sm truncate pr-2 text-slate-900"><span className="notranslate">{item.name}</span></h4>
  ...
  const renderProductModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-end justify-center sm:items-center p-0 sm:p-4">
        <div className="absolute inset-0 backdrop-blur-sm transition-opacity cursor-pointer bg-black/60" onClick={() => setSelectedProduct(null)} />
        <div className="relative w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl z-10 overflow-hidden animate-in slide-in-from-bottom duration-300 max-h-[90dvh] flex flex-col bg-white">
          <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 p-2 rounded-full shadow-lg z-20 transition cursor-pointer bg-white/90 backdrop-blur text-slate-800 hover:bg-white">
            <X size={20} />
          </button>
          ...
          <div className="overflow-y-auto pb-safe">
            {selectedProduct.image_url ? (
              ...
              <div className="w-full h-28 flex items-center justify-center font-bold text-xs bg-slate-100 text-slate-400"><span className="notranslate">Görsel Yok</span></div>
            )}
            
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-start gap-4">
                <h2 className="text-xl sm:text-2xl font-black leading-tight text-slate-900"><span className="notranslate">{selectedProduct.name}</span></h2>
              ...
              {selectedProduct.description && (
                <p className="text-xs sm:text-sm leading-relaxed font-medium p-4 rounded-2xl border text-slate-600 bg-slate-50 border-slate-100">
              ...
              {selectedProduct.allergens && selectedProduct.allergens.length > 0 && (
                <div className="pt-2">
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-2.5 text-slate-400"><span className="notranslate">{t.allergens}</span></p>
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.allergens.map((algId: string) => {
                      const alg = ALLERGEN_OPTIONS.find(a => a.id === algId);
                      if(!alg) return null;
                      return (
                        <div key={algId} className="flex items-center gap-2 px-3 py-1.5 rounded-xl border bg-slate-100 border-slate-200">
      ...
      <div className="fixed inset-0 z-50 flex">
        ...
        <div className="relative w-72 h-[100dvh] shadow-2xl z-10 flex flex-col font-sans animate-in slide-in-from-left duration-200 bg-white">
          ...
          <div className="p-4 border-b space-y-1 bg-slate-50 border-slate-100">
            ...
            <p className="text-[11px] text-slate-500"><span className="notranslate">{restaurant.subtitle || t.defaultSubtitle}</span></p>
          </div>

          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            <p className="text-[10px] font-bold uppercase px-2 mb-2 tracking-wider text-slate-400"><span className="notranslate">{t.categories}</span></p>
          ...
          <div className="p-4 border-t space-y-1 pb-safe bg-slate-50 border-slate-100 text-slate-500">
  ...
  // ============================================================================
  // ŞABLON 2: ÖZEL GRID TEMA (GÖNDERDİĞİN RESİMDEKİ KARE/AÇIK TEMA)
  // ============================================================================
      ...
      <div className="min-h-[100dvh] bg-slate-50 text-slate-900 font-sans pb-28 relative overflow-x-hidden">
        ...
        <div className="bg-white p-5 shadow-sm border-b">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5"><span className="notranslate">{restaurant.subtitle || t.defaultSubtitle}</span></p>
            </div>
          </div>
          <div className="flex flex-wrap gap-4 text-xs text-slate-500 pt-4 border-t mt-4 font-medium">
        ...
        {selectedCat === 'all' ? (
          <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-4">
            <h2 className="text-xs uppercase font-extrabold text-slate-400 tracking-wider mb-2"><span className="notranslate">{t.categoriesTitle}</span></h2>
          ...
          <div className="max-w-3xl mx-auto p-4 md:p-6 space-y-4">
            ...
            >
              <span className="notranslate">{t.backToCategories}</span>
            </button>
                  ...
                  <div className="flex-1 flex flex-col justify-between space-y-2">
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h3 className="font-extrabold text-base text-slate-900 leading-snug"><span className="notranslate">{prod.name}</span></h3>
                    ...
                    {prod.allergens && prod.allergens.length > 0 && (
                      <div className="flex items-center gap-2 pt-2 border-t border-slate-50">
                        <span className="text-[10px] font-bold text-slate-400"><span className="notranslate">{t.allergens}</span></span>
                        {prod.allergens.map((algId: string) => {
                          const alg = ALLERGEN_OPTIONS.find(a => a.id === algId);
                          return alg ? <span key={algId} className="text-sm notranslate" title={alg.label}>{alg.icon}</span> : null;
                        })}
                      </div>
                    )}
                  </div>
  ...
  // ============================================================================
  // ŞABLON 3: KLASİK TEMA (ORTALANMIŞ YUVARLAK LOGO)
  // ============================================================================
  if (template === 'classic') {
    return (
  ...
  // ============================================================================
  // ŞABLON 4: BİSTRO / MODERN TEMA (AKORDEONLU VARSAYILAN)
  // ============================================================================
  return (
    <div className="min-h-[100dvh] bg-slate-100 text-slate-900 font-sans pb-28 relative overflow-x-hidden">
      {renderCartDrawer()}
