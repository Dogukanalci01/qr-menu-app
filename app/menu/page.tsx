...
export default function PublicMenu() {
      ...
      <div className="fixed inset-0 z-50 flex">
        ...
        <div className="relative w-72 bg-white h-[100dvh] shadow-2xl z-10 flex flex-col font-sans animate-in slide-in-from-left duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            <p className="text-[10px] font-bold text-slate-400 uppercase px-2 mb-2 tracking-wider"><span className="notranslate">{t.categories}</span></p>
            <button onClick={() => { setSelectedCat('all'); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-between cursor-pointer ${selectedCat === 'all' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-100'}`}>
              ...
              <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded-full"><span className="notranslate">{products.length}</span></span>
            </button>
            ...
            {categories.map(cat => {
              ...
              return (
                <button key={cat.id} onClick={() => { setSelectedCat(cat.id); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-between cursor-pointer ${selectedCat === cat.id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-100'}`}>
                  ...
                  <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded-full"><span className="notranslate">{count}</span></span>
                </button>
              );
            })}
          </div>
          ...
          <div className="p-4 border-t text-[11px] text-slate-500 space-y-1 bg-slate-50 pb-safe">
            {restaurant.working_hours && <p className="flex items-center gap-1.5"><Clock size={12} /> <span className="notranslate">{restaurant.working_hours}</span></p>}
  ...
  // ============================================================================
  // ŞABLON 2: DN ÖZEL TEMASI (FOTOĞRAFLARINDAKİ 2 AŞAMALI ŞABLON)
  // Grid Kategoriler -> Tıklayınca Üstte Yatay Menü + Ürün Listesi
  // ============================================================================
  if (template === 'custom_grid') {
    return (
      <div className="min-h-[100dvh] bg-slate-50 text-slate-900 font-sans pb-28 relative overflow-x-hidden">
        ...
        {selectedCat === 'all' ? (
          // 1. AŞAMA: KATEGORİLER GRİDİ (Senin Attığın İlk Fotoğraf)
          <>
            ...
            <div className="bg-white p-5 shadow-sm border-b">
              <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-4">
                  {restaurant.logo_url && (
                    <img src={restaurant.logo_url} alt="Logo" className="w-16 h-16 rounded-full object-cover shadow-sm border border-slate-200" />
                  )}
                  <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-xs text-slate-500 pt-4 border-t mt-4 font-medium">
            ...
            <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-4 mt-2">
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-wider text-center sm:text-left"><span className="notranslate">{t.categoriesTitle}</span></h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {categories.map((cat) => (
                  <div 
                    ...
                    style={{ backgroundImage: cat.image_url ? `linear-gradient(to top, rgba(0,0,0,0.9), transparent), url(${cat.image_url})` : 'linear-gradient(to top, rgba(0,0,0,0.9), rgba(0,0,0,0.4))' }}
                  >
                    <span className="font-extrabold text-sm text-white relative z-20 uppercase"><span className="notranslate">{cat.name}</span></span>
                  </div>
                ))}
              </div>
            </div>
          ...
          // 2. AŞAMA: ÜRÜN LİSTESİ + YATAY MENÜ (Senin Attığın "2. Hali" Dediğin Kısım)
          <div className="w-full">
            {/* Yatay Kaydırmalı Kategori Barı */}
            <div className="bg-white shadow-sm sticky top-[60px] z-30 border-b border-slate-200">
              <div className="max-w-4xl mx-auto overflow-x-auto whitespace-nowrap flex gap-2 p-3 px-4 scrollbar-hide items-center">
                 <button 
                   ...
                   className="px-4 py-2 rounded-full text-xs font-bold transition border border-slate-200 text-slate-600 hover:bg-slate-50"
                 >
                   <span className="notranslate">{t.allProducts}</span>
                 </button>
                 {categories.map(cat => (
                   <button 
                     key={cat.id} 
                     onClick={() => setSelectedCat(cat.id)} 
                     className={`px-4 py-2 rounded-full text-xs font-bold transition border ${selectedCat === cat.id ? 'text-white border-transparent shadow-md' : 'text-slate-600 border-slate-200 hover:bg-slate-50'}`} 
            ...
            <div className="max-w-3xl mx-auto p-4 space-y-4 mt-2">
              {filteredProducts.map((prod) => (
                <div 
                  ...
                  className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row gap-4 cursor-pointer hover:shadow-md transition"
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between space-y-2">
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h3 className="font-extrabold text-base text-slate-900 leading-snug uppercase"><span className="notranslate">{prod.name}</span></h3>
                        <span className="font-black text-sm px-3 py-1 rounded-lg text-white whitespace-nowrap" style={{ backgroundColor: pColor }}>₺<span className="notranslate">{prod.price}</span></span>
                      </div>
