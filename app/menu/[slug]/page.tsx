...
export default function PublicMenu() {
  ...
  // GOOGLE TRANSLATE ZIRHI (Tıklamaların Bozulmaması İçin)
  useEffect(() => {
    fetchData();
  ...
  const fetchData = async () => {
    setLoading(true);
    // NOT: Eğer dinamik route (slug) kullanıyorsan buradaki fetch sorgusunu ona göre uyarlamalısın
  ...
  // MÜKEMMEL BÜYÜYEN ÜRÜN MODALI (TÜM ŞABLONLARDA ÇALIŞIR, TRANSLATE BOZAMAZ)
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex items-end justify-center sm:items-center p-0 sm:p-4">
        ...
        <div className={`relative w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl z-10 overflow-hidden animate-in slide-in-from-bottom duration-300 max-h-[90dvh] flex flex-col ${isDark ? 'bg-[#151D2C] border border-[#1E293B]' : 'bg-white'}`}>
          ...
          <button onClick={() => setSelectedProduct(null)} className={`absolute top-4 right-4 p-2 rounded-full shadow-lg z-20 transition cursor-pointer ${isDark ? 'bg-black/50 text-white hover:bg-black/70 border border-white/10' : 'bg-white/90 text-slate-800 hover:bg-white'}`}>
            <X size={20} />
          </button>
          ...
          <div className="overflow-y-auto pb-safe">
            {selectedProduct.image_url ? (
              ...
              <div className={`w-full h-32 flex items-center justify-center font-bold text-xs ${isDark ? 'bg-[#0B1120] text-slate-600' : 'bg-slate-100 text-slate-400'}`}><span>Görsel Yok</span></div>
            )}
            
            <div className="p-6 space-y-5">
              <div className="flex justify-between items-start gap-4">
                <h2 className={`text-2xl font-black leading-tight uppercase ${isDark ? 'text-white' : 'text-[#1e293b]'}`}><span>{selectedProduct.name}</span></h2>
                <p className="text-2xl font-extrabold whitespace-nowrap" style={{ color: isDark ? '#818cf8' : pColor }}>₺<span>{selectedProduct.price}</span></p>
              </div>
              
              {selectedProduct.description && (
                <div className={`p-4 rounded-2xl border ${isDark ? 'bg-[#0B1120] border-[#1E293B] text-slate-300' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
              ...
              {selectedProduct.allergens && selectedProduct.allergens.length > 0 && (
                <div className="pt-2">
                  <p className={`text-[10px] font-black uppercase tracking-wider mb-2.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}><span>{t.allergens}</span></p>
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.allergens.map((algId: string) => {
                      const alg = ALLERGEN_OPTIONS.find(a => a.id === algId);
                      if(!alg) return null;
                      return (
                        <div key={algId} className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${isDark ? 'bg-[#0B1120] border-[#1E293B]' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <button 
                ...
                className="w-full py-4 mt-2 rounded-2xl text-white font-black text-base shadow-lg flex items-center justify-center gap-2 hover:opacity-90 transition active:scale-95 cursor-pointer" 
                ...
                {cart.map((item) => (
                  <div key={item.id} className={`p-3 rounded-2xl flex gap-3 items-center border ${isDark ? 'bg-[#151D2C] border-[#1E293B]' : 'bg-white border-slate-100 shadow-sm'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-extrabold text-sm" style={{ color: isDark ? '#818cf8' : pColor }}>₺<span className="notranslate">{item.price * item.quantity}</span></p>
                    </div>
  ...
  const renderHeader = (isDark: boolean = false) => (
    <header className={`p-3 px-4 flex justify-between items-center shadow-md relative z-40 sticky top-0 ${isDark ? 'bg-[#0F172A] border-b border-[#1E293B] text-white' : 'text-white'}`} style={isDark ? {} : { backgroundColor: pColor }}>
        ...
        <div className={`relative w-72 h-[100dvh] shadow-2xl z-10 flex flex-col font-sans animate-in slide-in-from-left duration-200 ${isDark ? 'bg-[#0F172A]' : 'bg-white'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            ...
            <button onClick={() => { setSelectedCat('all'); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-between cursor-pointer ${selectedCat === 'all' ? (isDark ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-600') : (isDark ? 'text-slate-300 hover:bg-[#1E293B]' : 'text-slate-700 hover:bg-slate-100')}`}>
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${isDark ? 'bg-[#334155] text-white' : 'bg-slate-200'}`}><span>{products.length}</span></span>
            </button>
            ...
            {categories.map(cat => {
              ...
              return (
                <button key={cat.id} onClick={() => { setSelectedCat(cat.id); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-between cursor-pointer ${selectedCat === cat.id ? (isDark ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-600') : (isDark ? 'text-slate-300 hover:bg-[#1E293B]' : 'text-slate-700 hover:bg-slate-100')}`}>
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${isDark ? 'bg-[#334155] text-white' : 'bg-slate-200'}`}><span>{count}</span></span>
                </button>
              );
            })}
          </div>
  ...
  // ============================================================================
  // ŞABLON 1: DN ÖZEL TEMASI (custom_grid) -> SENİN RESİMDEKİ KOYU/YATAY MENÜ
  // ============================================================================
  if (template === 'custom_grid') {
    return (
      <div className="min-h-[100dvh] bg-[#0A0F1C] text-white font-sans pb-28 relative overflow-x-hidden">
        ...
        {/* RESTORAN KAPAK RESMİ & LOGO - ORTALANMIŞ */}
        {restaurant.cover_image && (
          <div className="w-full h-48 bg-cover bg-center relative" style={{ backgroundImage: `url(${restaurant.cover_image})` }}>
            <div className="absolute inset-0 bg-gradient-to-t from-[#0A0F1C] via-transparent to-transparent"></div>
          </div>
        )}

        <div className="flex flex-col items-center justify-center px-4 space-y-2 -mt-12 relative z-10">
          {restaurant.logo_url && (
            <img src={restaurant.logo_url} alt="Logo" className="w-24 h-24 rounded-2xl object-cover shadow-lg border-4 border-[#0A0F1C] bg-white" />
          )}
          <h1 className="text-2xl font-black tracking-tight mt-2 text-white"><span>{restaurant.name}</span></h1>
          <p className="text-xs text-slate-400 font-medium"><span>{restaurant.subtitle || t.defaultSubtitle}</span></p>
          <div className="flex items-center gap-4 text-[11px] text-slate-500 pt-2">
            {restaurant.working_hours && <span className="flex items-center gap-1"><Clock size={12} /> <span className="notranslate">{restaurant.working_hours}</span></span>}
            {restaurant.address && <span className="flex items-center gap-1"><MapPin size={12} /> <span>{restaurant.address}</span></span>}
          </div>
        </div>

        {/* YATAY KAYDIRMALI KATEGORİ MENÜSÜ */}
        <div className="sticky top-[60px] z-30 bg-[#0A0F1C] border-y border-[#1E293B] mt-6">
          <div className="max-w-4xl mx-auto overflow-x-auto whitespace-nowrap flex gap-2 p-3 px-4 scrollbar-hide">
            <button 
              onClick={() => setSelectedCat('all')} 
              className={`px-5 py-2.5 rounded-full text-xs font-bold transition border ${selectedCat === 'all' ? 'border-transparent text-white' : 'border-[#1E293B] bg-[#151D2C] text-slate-400 hover:text-white'}`} 
            ...
            {categories.map((cat) => (
              <button 
                ...
                className={`px-5 py-2.5 rounded-full text-xs font-bold transition border ${selectedCat === cat.id ? 'border-transparent text-white' : 'border-[#1E293B] bg-[#151D2C] text-slate-400 hover:text-white'}`} 
        ...
        <main className="max-w-4xl mx-auto p-4 space-y-4">
          {filteredProducts.map((prod) => (
            <div 
              key={prod.id} 
              onClick={() => setSelectedProduct(prod)} // TIKLAYINCA DEV EKRANI AÇAR
              className="bg-[#151D2C] border border-[#1E293B] rounded-2xl p-4 flex flex-col sm:flex-row gap-4 cursor-pointer hover:border-slate-600 transition shadow-sm"
            >
              {prod.image_url ? (
                <img src={prod.image_url} alt={prod.name} className="w-full sm:w-36 h-48 sm:h-32 object-cover rounded-xl flex-shrink-0 bg-[#0B1120]" />
              ) : null}
              
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start gap-4">
                    <h3 className="font-extrabold text-base text-white leading-snug"><span>{prod.name}</span></h3>
                    <span className="font-black text-sm px-3 py-1.5 rounded-lg text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 whitespace-nowrap">
                      <span>{prod.price}</span> ₺
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 line-clamp-2 mt-2 leading-relaxed"><span>{prod.description}</span></p>
                </div>
                ...
                {prod.allergens && prod.allergens.length > 0 && (
                  <div className="flex items-center gap-1.5 pt-3">
                    {prod.allergens.map((algId: string) => {
                      const alg = ALLERGEN_OPTIONS.find(a => a.id === algId);
                      return alg ? <span key={algId} className="bg-[#0B1120] p-1.5 rounded border border-[#1E293B] text-[10px] grayscale opacity-70 notranslate" title={alg.label}>{alg.icon}</span> : null;
                    })}
                  </div>
                )}
              </div>
  ...
  // ============================================================================
  // ŞABLON 2: KLASİK TEMA (ORTALANMIŞ YUVARLAK LOGO, BEYAZ TEMA)
  // ============================================================================
  if (template === 'classic') {
    return (
  ...
  // ============================================================================
  // ŞABLON 4: BİSTRO TEMA (BEYAZ, AKORDEONLU YAPI - VARSAYILAN)
  // ============================================================================
  return (
    <div className="min-h-[100dvh] bg-slate-100 text-slate-900 font-sans pb-28 relative overflow-x-hidden">
      {renderCartDrawer(false)}
